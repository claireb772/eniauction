...
public class FakeData {
	...
	static Logger log = Logger.getLogger(FakeData.class);
	...
	public SoldArticles newFakeSoldArticles(int nbUsers, int nbCategory) throws ParseException {
		...
		SoldArticles sa = new SoldArticles(0, parser.get("title").getAsString(),
				parser.get("description").getAsString(), new Date(), dateFin, price, price, user, category, true);
		return sa;
	}
	...
	public Users newFakeUser() {
		...
		Users user = new Users(0, parser.get("username").getAsString(), parser.get("last_name").getAsString(),
				parser.get("first_name").getAsString(), parser.get("email").getAsString(),
				parser.get("phone_number").getAsString(), addressParser.get("street_name").getAsString(),
				addressParser.get("zip_code").getAsString(), addressParser.get("city").getAsString(),
				parser.get("password").getAsString(), parser.get("last_name").getAsString(), randCredit, 0, 1, true,
	...
	private Auction newFakeAuction(int userCount, int soldArticleCount) throws Exception {
		...
		Auction auction = new Auction(new Random().nextInt(userCount) + 1, sa.getArticle_nb(), new Date(),
	...
	private Categories newFakeCategory() {
		...
		JsonObject parser = new JsonParser().parse(data).getAsJsonObject();
		Categories category = new Categories(0, parser.get("word").getAsString());
		return category;
	}
	...
	private Withdrawals newFakeWithDrawals(int nbArticle) {
		...
		if (!ma.isWithdrawalsExist(article)) {
			Withdrawals wd = new Withdrawals(article, addressParser.get("street_name").getAsString(),
					addressParser.get("zip_code").getAsString(), addressParser.get("city").getAsString());
			return wd;
		} else {
			return null;
